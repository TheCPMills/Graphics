...
public class glm {
    ...
    public static mat4 ortho(float width, float height, float near, float far) {
        return cuboid(-width / 2f, width / 2f, -height / 2f, height / 2f, near, far);
    }

    public static mat4 cuboid(float left, float right, float bottom, float top, float near, float far) {
        mat4 cuboid = new mat4();

        cuboid.set(0, new vec4(2f / (right - left), 0f, 0f, 0f));
        cuboid.set(1, new vec4(0f, 2f / (top - bottom), 0f, 0f));
        cuboid.set(2, new vec4(0f, 0f, -2f / (far - near), 0f));
        cuboid.set(3, new vec4(-(right + left) / (right - left), -(top + bottom) / (top - bottom), -(far + near) / (far - near), 1f));
