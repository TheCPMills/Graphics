...
public class Model {
    ...
    private void parseOBJFile(String fileName) throws Exception {
        ...
        // get all objects groups in the object
        Map<String, Obj> objectGroups = ObjSplitting.splitByGroups(object);
        for (Entry<String, Obj> entry : objectGroups.entrySet()) {
            String objectName = entry.getKey();
            Obj objectGroup = entry.getValue();

            // get active material for this group
            // match up to the first non-letter character
            String materialName = objectName.split("[^a-zA-Z]")[0];
            ...
            // construct mesh from object group
            Mesh mesh = construct3DMesh(objectGroup, material);
            meshes.add(mesh);
        }
    ...
    private Mesh construct3DMesh(Obj materialGroup, Mtl materialObject) {
        ...
        for (i = 0; i < vertices.capacity() / 3; ++i) {
            ...
            Vertex vertex = new Vertex(new vec3(x, y, z), new vec3(nx, ny, nz), new vec2(u, v));
        ...
        vec3 diffuse = new vec3(diffuseColor.get(0), diffuseColor.get(1), diffuseColor.get(2));
        ...
        Material material = new Material(ambient, diffuse, specular, emissive, shininess, refractionIndex, opacity);
