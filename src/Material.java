...
public class Material {
    ...
    public Material(vec3 ambientColor, vec3 diffuseColor, vec3 specularColor, vec3 emissiveColor, Float shininess, Float refractionIndex, Float opacity) {
    ...
    public void setUniforms(Shader shader) throws Exception {
        if (ambientColor == null || diffuseColor == null || specularColor == null || shininess == null) {
            throw new Exception("Material is not fully defined. Please specify at least the ambient color, diffuse color, specular color, and shininess of the material.");
        }

        glUniform3f(glGetUniformLocation(shader.id, "materialAmbient"), ambientColor.x, ambientColor.y, ambientColor.z);
        glUniform3f(glGetUniformLocation(shader.id, "materialDiffuse"), diffuseColor.x, diffuseColor.y, diffuseColor.z);
